        ...
        {QUICK_LINKS.map((link, i) => (
          ...
          >
            <TiltCard>
              <Link
                to={link.to}
                className={`group block rounded-3xl bg-gradient-to-br ${link.color} p-6 md:p-7 shadow-card hover:shadow-card-hover transition-shadow border border-white/40 h-full`}
              >
                <div className="text-4xl md:text-5xl mb-3 group-hover:scale-110 group-hover:rotate-3 transition-transform inline-block [transform:translateZ(30px)]">
                  {link.icon}
                </div>
