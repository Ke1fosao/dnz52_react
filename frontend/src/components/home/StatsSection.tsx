...
export function StatsSection() {
  ...
  return (
    <section className="py-16 md:py-20 bg-gray-900 dark:bg-slate-950 relative overflow-hidden">
      {/* Декоративний фон */}
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        <Reveal variant="fade-up" className="text-center mb-12">
          <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight mb-3">
            Наш заклад у <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">цифрах</span>
          </h2>
          <p className="text-gray-400 text-lg font-medium">Понад 40 років турботи про дітей Рівного</p>
        </Reveal>

        <Reveal stagger className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {STATS.map((stat, i) => (
            <Reveal key={stat.label} variant="scale" delay={i * 0.1}>
              <SpotlightCard className="premium-glass rounded-3xl h-full">
                <div className="p-6 md:p-8 text-center group hover:-translate-y-2 transition-transform duration-300 relative z-10">
                  ...
                  <div className="text-gray-400 font-bold text-sm md:text-base uppercase tracking-widest">{stat.label}</div>
                </div>
              </SpotlightCard>
            </Reveal>
          ))}
