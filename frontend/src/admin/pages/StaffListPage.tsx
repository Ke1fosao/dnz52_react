  ...
  return (
    <div className="space-y-5 animate-page-fade-in">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-3xl font-black text-gray-900 dark:text-white">Керівництво / Штат</h1>
          <p className="text-gray-500 dark:text-slate-400 font-medium">Адміністрація та працівники закладу</p>
        </div>
            ...
            <div key={s.id} className="premium-glass rounded-[1.5rem] p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  {s.is_featured && <span className="text-[11px] font-black px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 inline-flex items-center gap-1"><Star size={10} className="fill-current" /> Головна особа</span>}
                  {!s.is_active && <span className="text-[11px] font-black px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 dark:bg-slate-800 dark:text-slate-400 uppercase">Прихований</span>}
                </div>
                <h3 className="font-black text-gray-900 dark:text-white truncate">{s.full_name}</h3>
                <p className="text-xs text-gray-400 dark:text-slate-500 truncate">{s.position}</p>
              </div>
