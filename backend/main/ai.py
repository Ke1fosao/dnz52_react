...
_KIND_GUIDE = {
    ...
    'diet':    ('короткий опис користі щоденного раціону', 'Пиши звичайним текстом БЕЗ HTML-тегів. Максимум 2-3 речення (до 250 символів). Зосередься на вітамінах, енергії та користі для дітей.'),
}

_TONE_GUIDE = {
    'official': 'Пиши в офіційно-діловому, стриманому стилі, чітко по фактах, без зайвих емоцій та майже без емодзі.',
    'warm': 'Пиши дуже тепло, емоційно та доброзичливо. Використовуй радісні емодзі, щоб передати атмосферу дитячого садка.',
    'dynamic': 'Пиши коротко, жваво та динамічно. Передай суть у 2-3 яскравих реченнях з мінімумом тексту.'
}

_GEN_SYSTEM = (
    'Ти — копірайтер сайту дитячого садка ЗДО №52 (м. Рівне). Пишеш українською.\n'
    'СТРОГА ВКАЗІВКА ЩОДО ТОНУ: {tone_instruction}\n\n'
    'ФОРМАТ — лише готовий HTML-фрагмент (БЕЗ <html>, <body>, <h1> і БЕЗ markdown-огорож ```).\n'
    'Пиши структуровано:\n'
    '• Вступ — абзац <p class="lead">…</p>.\n'
    '• Смислові секції із заголовками <h3>…</h3>.\n'
    '• Де доречно — списки <ul><li>…: кожен пункт = <strong>ключове</strong> — короткий опис.\n'
    '• Для думки/цитати — <blockquote>«…», — підпис.</blockquote>\n'
    '• Важливе — <strong>, відтінки — <em>.\n\n'
    'ПРИКЛАД СТРУКТУРИ (адаптуй зміст і стиль під завдання та обраний тон):\n'
    '<p class="lead">У старших групах ЗДО №52 тривають <strong>заняття з підготовки до школи</strong>.</p>\n'
    '<h3>Наш підхід</h3>\n'
    '<p>Ми переконані: <strong>школа — це захоплива пригода</strong>, а не іспит.</p>\n'
    '<ul>\n'
    ...
    '</ul>\n'
    '<blockquote>«Найважливіше — щоб дитина прийшла до школи з радістю», — вихователь.</blockquote>'
)


def generate_text(brief: str, kind: str = 'generic', tone: str = 'warm') -> str:
    ...
    if kind == 'diet':
        system_prompt = (
            ...
            'Пиши одним коротким абзацом (2-3 речення).\n'
            'ОБОВ\'ЯЗКОВО: Проаналізуй конкретні страви з меню, яке тобі передадуть. '
            'Згадай в тексті 1-2 конкретні страви чи продукти з цього меню та опиши їхню користь (наприклад, вітаміни). '
            'Уникай занадто загальних фраз типу "Наш раціон - справжній скарб", пиши по суті сьогоднішнього меню.'
        )
    else:
...
_CHAT_SYSTEM = (
    ...
    'При цьому ОБОВ\'ЯЗКОВО додай у самому кінці своєї відповіді (з нового рядка) спеціальний тег [NOT_FOUND].\n'
    'Якщо ж ти зміг дати відповідь використовуючи контекст — тег [NOT_FOUND] НЕ ДОДАВАЙ.\n'
...
def analyze_chat_logs(questions_data: list, hide_answered: bool = False) -> str:
    ...
    if hide_answered:
        ...
        prompt += "   - **Низький пріоритет (Зелена зона)**: Всі інші питання (рідкісні і знайдені).\n"

    prompt += "\nОБОВ'ЯЗКОВО ПИШИ ВЕСЬ ЗВІТ УКРАЇНСЬКОЮ МОВОЮ.\n"
    prompt += "Формат: Markdown. Не використовуй <h1> (один #), починай з <h2> (два ##). Додай короткий вступ і висновки.\n"
